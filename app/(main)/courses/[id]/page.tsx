...
export default async function CourseDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{course.title}</h1><div className="flex gap-2"><Link href="/courses"><Button variant="outline" size="sm">Back</Button></Link><Button size="sm">Publish</Button></div></div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card><CardHeader><CardTitle className="text-base">Curriculum</CardTitle></CardHeader><CardContent className="space-y-4">
            ...
            <Button variant="outline" size="sm">+ Add Module</Button>
          </CardContent></Card>
        </div>

        <div className="space-y-6">
          <Card><CardHeader><CardTitle className="text-base">Settings</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="flex justify-between"><span className="text-muted">Status</span><span>{course.status}</span></div><div className="flex justify-between"><span className="text-muted">Category</span><span>{course.category}</span></div><div className="flex justify-between"><span className="text-muted">Level</span><span>{course.level}</span></div></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-base">Publish to Marketplace</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-xs text-muted">Submit for review to appear in marketplace</p><Button size="sm" className="w-full">Submit for review</Button></CardContent></Card>
        </div>
      </div>
    </div>
  );
